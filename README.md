...
sends `ui_state` updates back to the firmware for the screen. When `side_enter = true`, the side button sends Return once after a recognized result has been pasted; when `primary_enter = true`, releasing a front-button press shorter than 0.65 seconds does the same, while holding the button starts push-to-talk after the gesture window. Either manual-send option is mutually exclusive with `auto_enter`; `primary_enter` is also mutually exclusive with click-to-talk mode. All three send modes can be disabled.
